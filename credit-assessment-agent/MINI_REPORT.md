...
## 6. AI Assistant Usage (Streamlit layer)

Per the assignment, this notes where an AI coding assistant was used and what was changed by hand. The assistant was used mainly for the **Streamlit UI** (`app/streamlit_app.py`); the agent core, tools, and guardrails were designed and reviewed manually.

### What the assistant generated

- The initial Streamlit scaffold: page config, sidebar company selector, the `Run Assessment` button, and the main verdict/expander layout.
- Boilerplate rendering helpers — `_recommendation_ui` (colour-coded verdict), `_confidence_badge`, `_render_tool_outputs` (metric tables), and `_render_reasoning_steps` (LangGraph trace).
- First-pass error-handling blocks and the user-facing hint text in `_show_error`.

### What I adjusted or added by hand

- **Wired the UI to the real agent contract.** Mapped `AssessmentResult` fields into the panels, and made the tool-output renderer skip `data_quality_issues` and surface it as a caption instead.
- **Guardrail banner.** Added the prominent `st.warning` override banner and the "confidence forced to High" caption so an override is visually unmistakable — this is central to the assignment's auditability requirement, not something the scaffold produced.
- **Agent lifecycle / caching.** Replaced naive `@st.cache_resource` on the agent with a deliberately non-cached `get_agent` plus a one-time cache clear, after a stale `ChatOpenAI` client (still sending `max_tokens`) caused gpt-5 400 errors. Kept `@st.cache_resource` only for the data loader.
- **Robustness.** Added the Windows `WindowsSelectorEventLoopPolicy` fix, lazy/deferred agent imports so import failures render in the UI rather than crashing, and `DATA_PATH` resolution against the project root.
- **Reasoning transparency.** Ensured the reasoning-steps expander shows each `ToolMessage` in call order plus the pre-guardrail LLM rationale, so analysts see *why*, per the assignment.

All AI-generated code was read and tested before inclusion; the agent, guardrail, and tool logic (`agent/`) were written and reviewed independently of the UI scaffold.
